...
ITCM_FUNC void ecu_process(void)
{
  ...
  float periods_relation_filtered;
  float throttle;
  ...
  if(!gLocalParams.PhaseSimulation && !phased_injection) {
    if(throttleStatus == HAL_OK && running) {
                ...
                if(phased_injection) {
                  ...
                  for(int j = 0, cy; j < 2; j++) {
                    ...
                    if(!phased_detect_step || cy == 1 || cy == 3) {
                      ecu_inject(cy_count_injection * 2, cy, cy_injection[i]);
                    }
